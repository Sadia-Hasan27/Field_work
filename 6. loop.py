...
temp = [-15, 40,22,19,34,25,11,30]
th = 25
for i in temp:
    if i > th:
        print(f"Warning: Temperature {i}°C is above the threshold of {th}°C")
...
for i, j in products.items():
    ...
    if st < req:
        print(f"Warning!: {i.capitalize()} stock is low. This product need to restock minimum {req}")
    else:
        print(f"{i.capitalize()} is available in stock")
